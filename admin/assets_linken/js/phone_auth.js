...
function canClickSendNoBoundCode() {
    ...
    $('#verification_code').unbind("click").bind("click", function () {
        ...
        if ($.trim(phone) == "") {
            maskShowAlert('Please enter the phone number you want to bind!');//请输入您要绑定的手机号!
            return;
        }
        var regPhone = /^[1][3,4,5,7,8][0-9]{9}$/;
        if (!phone.match(regPhone)) {
            maskShowAlert("The phone number is INVALID!")//手机号格式有误!
            return;
        }
        $.ajax({
            type: 'POST',
            ...
            success: function (data) {
                if (data.error_code != 0) {
                    if (data.error_code == 204) {
                        maskShowAlert('This phone number has already been bound, please confirm!');//此手机号已绑定,请确认!
                        return;
                    } else if (data.code == 35) {
                        loginOvertime('login.html?lp=' + GetUrlRelativePath());
                    } else {
                        maskShowAlert('Fail in sending message verification code!')//短信验证码发送失败!
                        return;
                    }
                } else {
                    maskShowAlert('Send SMS successfully!');// 发送短信成功！
                    setTimeout(function () {
                        $(".dialog,.mask").hide();
                        getMobileCheckCode();
                    }, 2000);
...
function msgInterval() {
    ...
    if (interval == 0) {
        ...
        if (isNaN(interval) || isNaN(interval - 1)) {
            $(".get_verCode").val("Get mobile phone verification code");//获取手机验证码
        } else {
            interval = interval - 1;
            $(".get_verCode").text(interval+ "seconds resend");//interval + " 秒后重发"
        }
...
function phoneValidate() {
    ...
    if ($('#phone_vercode').val() == "") {
        maskShowAlert('Enter the verification code!');//请输入短信验证码!
        return;
    }
    $.ajax({
        type: 'POST',
        ...
        success: function (data) {
            if (data.error_code == 0) {
                ...
                maskShowAlert('Error random value check code!');//错误的随机值验证码!
...
function phoneUpdate() {
    if ($('#input_phone_password').val() == "") {
        maskShowAlert('Please enter your transaction password!');//请输入您的交易密码!
        return;
    }
    var bound_phone = $('#input_phone_bind').val().trim();
    if (bound_phone == "") {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.error_code == 0) {
                maskShowAlert('Modify phone number successfully!');//修改手机号成功!
                store.set("username", $('#input_phone_bind').val().trim());
                setTimeout(function () {
                ...
                maskShowAlert('Please fill in the correct transaction password!');//请填写正确的交易密码!
                return;
            } else if (data.error_code == 20) {
                maskShowAlert('This user has not set up the transaction password, please set!');//此用户暂未设置交易密码,请设置!
                return;
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.error_code == 0) {
                ...
                maskShowAlert('Error, please confirm!');//请求参数有误,请确认!
                return;
            } else if (data.error_code == 35) {
                loginOvertime('login.html?lp=' + GetUrlRelativePath());
