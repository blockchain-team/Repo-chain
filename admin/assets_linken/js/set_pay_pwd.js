...
function setPayPwd() {
    ...
    if (username != "" && signature != "" && username != undefined && signature != undefined) {
        ...
        if (payPwd == "") {
            $("#alert_info").text("Please enter transaction password!");//请输入交易密码!
            $(".dialog_init,.common_mask").show();
            return;
        }
        if (confirmPwd == "") {
            $("#alert_info").text("Please enter again!");//请重复输入交易密码!
            $(".dialog_init,.common_mask").show();
            return;
        }
        if (payPwd != confirmPwd) {
            $("#alert_info").text("Passwords should be the same!");//两次输入密码需一致!
            $(".dialog_init,.common_mask").show();
            return;
        }
        if (!/^\d{6}$/.test(payPwd)) {
            $("#alert_info").text("Transaction password should contain 6 numbers!");//交易密码只能为6位数字!
            $(".dialog_init,.common_mask").show();
            return;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.error_code == 0) {
                    $("#alert_info").text("Transaction password set successfully!");//交易密码设置成功!
                    $(".dialog_init,.common_mask").show();
                    loadAccount();
                }
                    ...
                    $("#alert_info").text("Account not found!");//用户不存在!
                    $(".dialog_init,.common_mask").show();
                    return;
                }
                else if (data.error_code == 57) {
                    $("#alert_info").text("Ransaction password already set. If need revise, please reset it!");//交易密码已设置 如需更改请进行重置!
                    $(".dialog_init,.common_mask").show();
                    return;
                }
                else if (data.error_code == 210) {
                    $("#alert_info").text("Password is not encrypted!");//密码未加密!
                    $(".dialog_init,.common_mask").show();
                    return;
                }
                else if (data.error_code == 35) {
                    $("#alert_info").text("Login timeout Please log in again!");//登录超时 请重新登录!
                    goHtml("set_trade_pwd.html");
                }
                else {
                    $("#alert_info").text("Unknown error, operation failed!");//未知错误 操作失败!
                    $(".dialog_init,.common_mask").show();
                    return;
                }
            }
        ...
        $("#alert_info").text("Please login!");//请登录!
        $(".dialog_init,.common_mask").show();
            ...
            if (data.error_code == 0) {
                if (data.setFlag == true) {
                    ...
                    $.each(questions, function (i, item) {
                        var question1 = $('<li class="checkquestions-item" data-value="' + item.id + '">' + item.question_en + '</li>');
                        $(".checkquestions-choose").append(question1);
                    });
                    $.each(questions, function (i, item) {
                        var question2 = $('<li class="checkquestionstwo-item" data-value="' + item.id + '">' + item.question_en + '</li>');
...
function checkUserQuestion1() {
    ...
    if (username != "" && username != undefined && signature != "" && signature != undefined) {
        ...
        if (answer == "") {
            showAlert("Please input the answer!");//请输入答案!
            return;
        }
        if(valuetwo == ""){
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.error_code == 0) {
                    ...
                    showAlert("Parameter exception!");//参数异常!
                }
            }
        });
    } else {
        showAlert("Please login!");//请登录!
        goHtml("login.html?lp=set_trade_pwd.html");
    }
}
...
function resetPayPwd() {
    ...
    if (username != "" && username != undefined && signature != "" && signature != undefined) {
        ...
        if (confPayPwd == "") {
            showAlert("Please repeat the transaction password!");//请重复交易密码!
            return;
        }
        if (resetPayPwd != confPayPwd) {
            showAlert("Two inputs need to be consistent!");//两次输入需保持一致!
            return;
        }
        if (!/^\d{6}$/.test(resetPayPwd)) {
            showAlert("Transaction password should contain 6 numbers!");//交易密码只能为6位数字!
            return;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.error_code == 0) {
                    showAlert("Transaction password reset successfully!");//交易密码重置成功!
                    setTimeout(function () {
