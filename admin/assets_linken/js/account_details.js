            ...
            success:function (data) {
                if (data.error_code!=0){
                    if (data.error_code==1){
                        showAlert("Query exception");
                        return;
                    }
                }
                if(data.data.balances.length==0){
                    return showAlert("No other assets!");
                }else {
                    var str=""
...
function loadaccount(json) {
    $.ajax({
        ...
        success:function (data) {
            if(data.error_code!=0){
                if(data.error_code==1)
                    showAlert("The address does not exist!",3);
            ...
            for (var i=data.data.tradingList.length-1;i>=0;i--){
                ...
                str+='<ul class="trans_data" >' +
                    '<li><a><span class="data_id topage" data-href="transaction_record.html?id='+item.id+'">'+item.id+'</span></a><span class="data_time fr">'+AschJS.utils.format.fullTimestamp(item.timestamp)+'</span><span class="data_num fr">{Transaction Fee：<em>'+item.fee+'ACC</em>}</span></li>' +
                    ...
                    '<p class="clearfix"><span class="fr"  style="'+style+'">'+acc+ type+'</span><span class="fr"><em>'+item.confirmations+'</em>Confirm</span></p></ul>'
               ...
               /* str+= ' <ul class="trans_data">' +
                    ...
                    '<p class="clearfix"> <span class="fr" style="'+style+'">'+acc+' '+type+'</span> <span class="fr"><em>'+item.confirmations+'</em>Confirm</span> </p> ' +
                    '</ul>';*/
            }
            $("#listView").append(str);
