    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.success){
                ...
                $("#award").html(data.block.reward+"&nbsp;ACC");
            ...
            if(data.count=="0"&&data.success){
               ...
               /* totalTransactionAmount(blockId,offset,data.count);*/
                ...
                if(data.transactions.length<10){
                    $(".show_more_records").text("没有更多数据")
                    load=0;
                }
                if(data.transactions.length>0){
                    if(data.count>10&&data.transactions.length>0){
                        load=1;
                    }
                    if(data.transactions.length==0){
                        load=0
                    ...
                    $.each(trans,function (i,item) {
                        ...
                        if(item.recipientId==""){
                           /* var transList=$("#transactionList").append('<ul class="trans_data"><li><span class="data_id"><a href="transaction_record.html?id='+item.id+'">'+item.id+'</a></span>'
                                ...
                                +'<span class="data_num fr">{交易费：<em>'+item.fee+'&nbsp;ACC</em>}</span></li>'
                                ...
                                +' </span></li><p class="clearfix"><span class="fr" >0&nbsp;'+currency+'</span><span class="fr"><em>'+item.confirmations+'</em>确认</span>'
                                +'</p></ul>')*/
                            var transList=$("#transactionList").append('<ul class="trans_data" >'
                                ...
                                +'<span class="fl td_img relative"><img src="img/data_arrow_grey.png" alt="" class="fl"/><em>(0 ACC)</em></span>'
                                +'<span class="fl tdi_txt"><p><em>'+revicer+'</em></p></span></li><p class="clearfix">'
                                +'<span class="fr">0&nbsp;'+currency+'</span><span class="fr"><em>'+item.confirmations+'</em>确认</span></p></ul>');
                            ...
                            /*var transList=$("#transactionList").append('<ul class="trans_data"><li><span class="data_id"><a href="transaction_record.html?id='+item.id+'">'+item.id+'</a></span>'
                                ...
                                +' </span></li><p class="clearfix"><span class="fr">'+item.amount+'&nbsp;'+currency+'</span><span class="fr" ><em>'+item.confirmations+'</em>确认</span>'
                                +'</p></ul>')*/
                            var transList=$("#transactionList").append('<ul class="trans_data" >'
                                +'<li><span class="data_id"><a href="transaction_record.html?id='+item.id+'">'+item.id+'</a></span><span class="data_time fr">'+AschJS.utils.format.fullTimestamp(item.timestamp).replace('/','-').replace('/','-')+'</span>'
                                +'<span class="data_num fr">{交易费：<em>'+item.fee+'&nbsp;ACC</em>}</span></li>'
                                ...
                                +'<span class="fr">'+item.amount+'&nbsp;'+currency+'</span><span class="fr"><em>'+item.confirmations+'</em>确认</span></p></ul>');
                        }
                        $("#transactionList").append(transList);
                    })
                ...
                load=0;
                $("#show_more_recordsId").text("没有更多数据")
            }
        }
    });
}
