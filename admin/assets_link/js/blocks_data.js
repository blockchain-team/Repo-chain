...
$(function(){
   ...
   /* $("#search").on("click",function () {
        ...
        if(content.length==34)
            ...
            showAlert("The input account is incorrect!");
        else if(content.length>34)
            showAlert("Entering transaction ID is incorrect");
...
//                        $("#block_detail_trans_total").text(getConversion(block.totalAmount));
...
//                            var blockInfo = $('<tr class="blocks_info" id="'+ blocks.id +'"><td class="height">'+ blocks.height +'</td><td>'+ getLocalTime(blocks.timestamp) +'</td><td>'+ blocks.id +'</td><td class="generatorId">'+ blocks.generatorId +'</td><td class = "transactions">'+ blocks.numberOfTransactions+'</td><td>'+ getConversion(blocks.totalFee) +'</td><td>'+ getConversion(blocks.reward) +'</td></tr>');
                            var blockInfo = $('<tr class="blocks_info" id="'+ blocks.id +'"><td class="height">'+ blocks.height +'</td><td>'+ AschJS.utils.format.fullTimestamp(blocks.timestamp) +'</td><td>'+ blocks.id +'</td><td class="generatorId">'+ blocks.generatorId +'</td><td class = "transactions">'+ blocks.numberOfTransactions+'</td><td>'+ blocks.totalFee +'</td><td>'+ blocks.reward +'</td></tr>');
                            $(".blocks_list").append(blockInfo);
                            getBlockDetail(id);
                            getGeneratorInfo(id);
                            getBlockTrans(id);
