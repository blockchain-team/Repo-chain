    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.error_code == 0){
                ...
                $("#ret_join").attr("data-login","true");
                $("#btc_join").attr("data-value",data.BTCAddress);
                $("#eth_join").attr("data-value",data.ETHAddress);
                $("#acc_join").attr("data-value",data.ACCAddress);
                $("#ret_join").attr("data-value",data.RETAddress);
                $("#go_ico_user").attr("href","acc_ico.html?id="+data.userId+"&category=SRT");
            }
        }
    });
}
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.error_code == 0){
                ...
                $("#retJoinCount").text(data.retJoinCount);
                ...
                var retRanking = data.retRankingList;
                $(".ret_ranking").remove();
                $.each(retRanking,function(i,item){
                    var ranking = $('<ul class=ret_ranking ><li>'+item.ranking+'</li><li>'+item.addrerss+'</li><li>'+item.amount+'</li></ul>');
                    $("#mCSB_4_container").append(ranking);
                });
